...
export default function ServiceToolsPage() {
      ...
      if (response) {
        response.forEach(item => {
          ...
          transformedToolPrompts[key] = {
            ...
            isCustom: true, // Assume custom for service API items
            description: '', // Not provided in API response
          }
        })
      }
      
      setToolPrompts(transformedToolPrompts)
    } catch (error) {
  ...
  const handleCreate = () => {
    navigate('/service/tools/editor/new')
  }

  const handleEdit = (toolName: string, connectorType: string) => {
    navigate(`/service/tools/editor/${toolName}:${connectorType}`)
  }
