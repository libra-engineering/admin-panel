...
import { libraryApi, type CreateWorkflowLibraryRequest } from '../services/libraryApi'
...
export default function WorkflowBuilder() {
  ...
  const handleSave = async () => {
    ...
    const payload: CreateWorkflowLibraryRequest = {
    ...
    try {
      if (workflowId) {
        await libraryApi.updateWorkflowTemplate(workflowId, payload)
        toast.success('Workflow updated')
      } else {
        const created = await libraryApi.createWorkflowTemplate(payload)
        toast.success(`Workflow "${created.name}" created`)
      }
      navigate('/admin/agents?tab=workflows')
    } catch (e) {
      console.error(e)
